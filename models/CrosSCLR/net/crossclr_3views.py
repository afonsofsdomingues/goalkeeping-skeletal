...
class CrosSCLR(nn.Module):
    ...
    def __init__(self, base_encoder=None, pretrain=True, feature_dim=128, queue_size=32768,
        ...
        if layout == 'native':
            # Native 29-Joint Skeleton Connectivity (1-based indices)
            # Connectivity: Child -> Parent
            self.Bone = [
                (2, 1), (18, 1), (19, 1),   # neck, hips -> midHip
                (3, 2), (8, 2), (9, 2),     # nose, shoulders -> neck
                (4, 3), (5, 3),             # eyes -> nose
                (6, 4), (7, 5),             # ears -> eyes
                (10, 8), (11, 9),           # elbows -> shoulders
                (12, 10), (13, 11),         # wrists -> elbows
                (14, 12), (15, 12),         # thumb, pinky -> wrist (L)
                (16, 13), (17, 13),         # thumb, pinky -> wrist (R)
                (20, 18), (21, 19),         # knees -> hips
                (22, 20), (23, 21),         # ankles -> knees
                (24, 22), (26, 22),         # bigToe, heel -> ankle (L)
                (25, 24),                   # smallToe -> bigToe (L)
                (27, 23), (29, 23),         # bigToe, heel -> ankle (R)
                (28, 27)                    # smallToe -> bigToe (R)
            ]
        else:
            # NTU 25-Joint Default
    ...
    def get_embeddings(self, im_q, view='joint', normalize=False):
        """
        Extract embeddings from input data
        
        Args:
            im_q: input tensor (N, C, T, V, M)
            view: 'joint', 'motion', 'bone', or 'all'
        ...
        with torch.no_grad():
            if view == 'joint':
                embeddings = self.encoder_q(im_q)
            elif view == 'motion':
                im_q_motion = torch.zeros_like(im_q)
                im_q_motion[:, :, :-1, :, :] = im_q[:, :, 1:, :, :] - im_q[:, :, :-1, :, :]
                embeddings = self.encoder_q_motion(im_q_motion)
            elif view == 'bone':
                im_q_bone = torch.zeros_like(im_q)
                for v1, v2 in self.Bone:
                    im_q_bone[:, :, :, v1 - 1, :] = im_q[:, :, :, v1 - 1, :] - im_q[:, :, :, v2 - 1, :]
                embeddings = self.encoder_q_bone(im_q_bone)
            elif view == 'all':
                im_q_motion = torch.zeros_like(im_q)
                im_q_motion[:, :, :-1, :, :] = im_q[:, :, 1:, :, :] - im_q[:, :, :-1, :, :]
                ...
                embeddings = (self.encoder_q(im_q) + 
                             self.encoder_q_motion(im_q_motion) + 
                             self.encoder_q_bone(im_q_bone)) / 3.0
            else:
                raise ValueError(f"Invalid view: {view}. Use 'joint', 'motion', 'bone', or 'all'")
